"""Schwab Trader API adapter: place, replace, cancel, flatten, read the record.

THE SCHWAB HALF OF `live/broker.py`, moved here unchanged when the second
broker (DAS) made an interface necessary. Everything in this file is about
speaking to Schwab. The things that are decisions about TRADING rather than
about Schwab -- the four switches, the guards, what a flatten does first --
stayed in the façade and are checked before any method here is reached; see
live/brokers/base.py for why that split is a safety property and not tidiness.

Three switches are named below because the module is arranged around them,
but this file no longer enforces them.
...
async def reconcile(*, symbol: str, side: str, qty: float,
    ...
    THE MATCH IS BY SHAPE (base.match_placement) BECAUSE SCHWAB LEAVES NO
    OTHER HANDLE. The caveat below is Schwab's, which is why it lives in
    Schwab's adapter: a broker whose API accepts a client order id should
    override this method and match on that instead.
...
# ── health ──────────────────────────────────────────────────────────────────
def health() -> dict:
    """Schwab's own facts. The trading switches are the façade's to report."""
    tokens = _read_tokens()
    return {
        "broker": "schwab",
...
# ── the adapter ─────────────────────────────────────────────────────────────
#
# METHODS DELEGATE TO THE MODULE FUNCTIONS ABOVE, by name, at call time. That
# is deliberate: the functions are what check_broker.py has always driven and
# monkeypatched (`schwab._acall`, `schwab._account_hash`), and binding them
# here rather than at import keeps that working -- a patched `_acall` is seen
# by the class too. It also keeps this file readable as "the Schwab protocol"
# with the class as a thin conformance layer over it.
class SchwabBroker(base.Broker):
    """Schwab Trader API. One account, equities, one connection."""
    ...
    async def state(self, symbols=None, priority=False):
        # Overridden: the two reads run CONCURRENTLY here, which is the whole

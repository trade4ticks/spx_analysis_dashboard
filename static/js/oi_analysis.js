...
document.addEventListener('alpine:init', () => {
  ...
  Alpine.data('oiAnalysis', () => ({
    ...
    _darkScales(...a) { return window.FactorCharts._darkScales(this, ...a); },
    ...
    _hmRecomputeRange(...a) { return window.FactorCharts._hmRecomputeRange(this, ...a); },
    ...
    rollingIcSubtitle() {
      ...
      // train_test: the reference is frozen on the pre-cutoff window and
      // stability counts ONLY post-cutoff (out-of-sample) windows. Label
      // both — a stability % whose denominator silently changes between
      // modes is worse than no number at all.
      const isTT    = p.stability_scope === 'post_cutoff';
      const stabLbl = isTT ? 'Stability (test window)' : 'Stability';
      const refTxt = `ref ${(p.reference_ic ?? 0).toFixed(3)}`
                   + (isTT ? ' (frozen pre-cutoff)' : '');
      ...
      if (ss.suppressed) {
        ...
        return `[${modeTxt}] ${stabLbl}: — (${reason}) · ${refTxt} · ${epsTxt}`;
      }
      ...
      return `[${modeTxt}] ${stabLbl}: ${stab} · ${neutPct}% neutral · ${refTxt} · ${epsTxt}`;
    },
    ...
    //                     curve's endpoint.
    _yearlyForMode(...a) { return window.FactorCharts._yearlyForMode(this, ...a); },

    _renderZoneYearly(...a) { return window.FactorCharts._renderZoneYearly(this, ...a); },
    ...
    // Per-cell shade + opacity delegate to the shared SignalThumb
    // globals (loaded from /static/js/signal_thumbnail.js BEFORE this
    // file). Single source of truth shared with the Signals page; a
    // cell shaded by cellColor and a thumbnail rectangle for the same
    // avg_ret literally execute the same function — no scale drift
    // possible between them. Existing call sites here keep working
    ...
    // Delegate to shared SignalThumb.thumbnailSVG so the FA and Signals
    // pages render byte-identical SVG for the same signal. Pre-existing
    // call sites here (Saved Signals list cards) keep their click
    // affordance via wrapping :style/@click on the host element, which
    ...
    _renderSecBubble(...a) { return window.FactorCharts._renderSecBubble(this, ...a); },
    ...
    _dedupeConcurrent(...a) { return window.FactorCharts._dedupeConcurrent(this, ...a); },
    ...
    _avgRetLinePlugin(...a) { return window.FactorCharts._avgRetLinePlugin(this, ...a); },
    ...
    _equityModeKey(...a) { return window.FactorCharts._equityModeKey(this, ...a); },
    ...
    _equityForMode(...a) { return window.FactorCharts._equityForMode(this, ...a); },
    ...
    _drawdownFromEquity(...a) { return window.FactorCharts._drawdownFromEquity(this, ...a); },
    ...
    _drawdownFromDollarEquity(...a) { return window.FactorCharts._drawdownFromDollarEquity(this, ...a); },
    ...
    _computeDollarSeries(...a) { return window.FactorCharts._computeDollarSeries(this, ...a); },

    _renderSecEquity(...a) { return window.FactorCharts._renderSecEquity(this, ...a); },
    ...
    _renderSecActivity(...a) { return window.FactorCharts._renderSecActivity(this, ...a); },

...
3. **Inline `/heatmap` single-ticker binning** — `/heatmap` single-ticker uses `np.percentile` edges (deliberately different algorithm; user-visible edge labels). Decided to keep — by-design exception. (`today_decile` inline formula migrated in Step 7i to use `pairs_decile[-1]` from the active Assigner.)
4. **Dead legacy helpers** — done in Step 7j. Relocated 13 single-caller helpers from `oi_analysis.py` to `row_compute.py` (`_walk_forward_bucket_pairs`, `_walk_forward_bucket_per_ticker`, `_bin_membership`, `_bin_for_value`, `_walk_forward_bins`, `_compute_bins_for_metric`, `_sort_chrono`, `_walk_forward_primary_filter`, `_compute_walk_forward_bin_stats`, `_walk_forward_membership`, `_compute_all_bins_fast`, `_compute_all_bins_walk_forward`, `_compute_all_bins_train_test_fast`). `_bucket_pairs` / `_bucket_pairs_per_ticker` / `_walk_forward_thresholds` stay in `oi_analysis.py` because they're still called by view-specific code (yearly_consistency, half-sample, threshold-drift). `oi_signals.py` now imports `_bin_for_value` from `row_compute`.
